...
def make_anchors(feats, strides, grid_cell_offset: float=0.5): #strides=[ 8., 16., 32.]
    ...
    for i, stride in enumerate(strides):
        ...
        sx = torch.arange(end=w, device=device, dtype=dtype) + grid_cell_offset  # shift x 05~79.5
...
def dist2bbox(distance, anchor_points, xywh=True, dim=-1): #distance:[16, 8400, 4], anchor_points:[8400, 2]
    """Transform distance(ltrb) to box(xywh or xyxy)."""
    lt, rb = distance.chunk(2, dim)
    x1y1 = anchor_points - lt #[16, 8400, 2]
    x2y2 = anchor_points + rb #[16, 8400, 2]
    if xywh: #False
        c_xy = (x1y1 + x2y2) / 2
        wh = x2y2 - x1y1
...
def bbox2dist(anchor_points, bbox, reg_max):#anchor_points:[8400, 2], bbox:[16, 8400, 4]
